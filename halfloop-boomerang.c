 ...
 * @param seed0 the base seed to use. Can be any arbitrary seed, but must be the same as used for
 * generating ct0 and pt0x.
 * @param pt0 the base plaintext to use. Can be any arbitrary plaintext.
 * @param ct0 pt0 encrypted using key and seed0.
 ...
 */
halfloop_result_t restore_byte(u128 key, u64 seed0, u32 pt0, u32 ct0, u32 pt0x, u8 beta, u8 gamma,
    u8 n, int *operations) {
  ...
  u8 shift = 16 - (n * 8);
  u64 seed1 = seed0 ^ ((u64)mix_columns(rotate_rows((u32)beta << shift)) << 16);

  for (int delta = 1; delta < 0x100; delta++) {
    u32 pt1 = pt0 ^ (delta << shift);
    u32 ct1;
    u32 pt1x;
    RETURN_ON_ERROR(halfloop_encrypt(pt1, key, seed1, &ct1));
    RETURN_ON_ERROR(halfloop_decrypt(ct1, key, seed1 ^ GAMMA_SHIFT(gamma), &pt1x));
    *operations += 1;
...
int main(int argc, char *argv[]) {
  ...
  pt0 &= 0xffffff;
  RETURN_ON_ERROR(random_bytes(&seed0, sizeof(u64)));
  RETURN_ON_ERROR(random_bytes(&key, sizeof(u128)));

  RETURN_ON_ERROR(halfloop_encrypt(pt0, key, seed0, &ct0));
  RETURN_ON_ERROR(halfloop_decrypt(ct0, key, seed0 ^ GAMMA_SHIFT(gamma), &pt0x));
  operations += 1;

  print_message("Key:        %016" PRIx64 "%016" PRIx64, WHITE, (u64)(key >> 64), (u64)key);
  print_message("Seed:       %016" PRIx64, WHITE, seed0);
  ...
  for (int i = 0; i < 3; i++) {
    RETURN_ON_ERROR(restore_byte(key, seed0, pt0, ct0, pt0x, beta, gamma, i,
        &operations));
  }
